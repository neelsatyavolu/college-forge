...
This is a **comparison of past graduates' outcomes** for students who received federal aid and graduated several years ago. Its program earnings estimates, as reported (before any cost-of-living adjustment), were checked in an exploratory retrospective evaluation: built only from earlier graduating classes, they predicted the next classes better than the alternatives tested, on institutions excluded from calibration and selection, and separate 90% next-class prediction intervals (which also include the next class's own sampling noise) held the next classes' results 91–94% of the time. Earlier results informed revisions, so this is not an untouched confirmatory test. The cost-of-living adjustment (applied at half weight by default), the narrower intervals behind published rank ranges, the rank ranges themselves and the composite weights are not validated. It is not a forecast for any individual student, and it does not measure what a college causes.
...
6. **Cost of living counts half by default; none and full are one click away.** The default divides earnings by the square root of the graduate price level: equal weight on what the degree earns and what that pay buys where graduates live. That weight is a value judgment, not an estimate, and it was chosen after the orderings had been compared. Prices come from BEA Regional Price Parities (plus extra housing weight). Where graduates work is observed from Census PSEO for 458 of 1,717 schools in the geography step (408 of 1,281 overall-ranked colleges) and estimated for the rest, and neither the estimates nor the observed-destination construction is externally validated. Against earnings as reported, the typical college moves about 39 places at half weight and 97 at full weight. Every row shows its rank in the other orderings. The as-reported ordering avoids these assumptions and is the one the backtest checks. State taxes are not deducted.
7. **Uncertainty.** Rank ranges (5th–95th percentile) redraw each earnings estimate and, in the cost-of-living views, the estimated price level (each estimator with its own held-out error). They describe uncertainty in each estimate, not the spread of a future class's result, and are conditional on the model; they are not empirically calibrated. (At their published width, program intervals contained the next class's result 83% of the time for bachelor's and 91% for master's in the backtest, as expected for intervals that leave out the next class's own sampling noise.) Weights and other specification choices are reported as sensitivity. Overlapping ranges are a reason not to over-read the exact order; they do not prove two schools are equal.
...
9. **Only published cells are ranked.** A school missing from a major table may not offer the major, may report it under a related code, may have suppressed earnings, or may rank below the top 250 shown in the active ordering. Search works within the published top 250.
...
4. A program is scored on 4-year earnings, or 5-year earnings (an older class) where the 4-year figure is suppressed (3,136 of 34,637 ranked programs). The 5-year national baseline is an earner-weighted median of program medians, since Scorecard publishes no official 5-year national median. A 5-year premium is shifted onto the 4-year scale by its major's typical gap where both are published (checked on held-out institutions, but only where both figures are visible; fallback programs are smaller), and the shift's uncertainty is added. These programs are labeled "older class", and colleges scored mostly on them are flagged; treat their positions as provisional. Programs with only 1-year earnings are not ranked.
"""
...
- Two-bucket price estimate (in-state share observed, destinations not): held-out log error {geo_diag['two_dest_model']['log_rmse_cv']:.3f} over {geo_diag['two_dest_model']['n_scored']:,} schools with both; used for {geo_diag['n_by_rpp_source'].get('pseo_instate', 0):,} schools
