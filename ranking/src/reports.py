...
6. **The page ranks after cost of living by default; this is an exploratory adjustment, and an as-reported ordering (no geography) is one click away.** The cost-of-living ordering adjusts with BEA Regional Price Parities (plus extra housing weight). Where graduates work is observed from Census PSEO for 458 of 1,717 schools in the geography step (408 of 1,281 overall-ranked colleges) and estimated for the rest, and neither the estimates nor the observed-destination construction is externally validated. The choice moves the typical college about 97 places. Every row shows its rank in the other ordering. The as-reported ordering avoids these assumptions and is the one the backtest checks. State taxes are not deducted.
7. **Uncertainty.** Rank ranges (5th–95th percentile) redraw each earnings estimate and, in the cost-of-living view, the estimated price level (each estimator with its own held-out error). They describe uncertainty in each estimate, not the spread of a future class's result, and are conditional on the model; they are not empirically calibrated. (At their published width, program intervals contained the next class's result 83% of the time for bachelor's and 91% for master's in the backtest, as expected for intervals that leave out the next class's own sampling noise.) Weights and other specification choices are reported as sensitivity. Overlapping ranges are a reason not to over-read the exact order; they do not prove two schools are equal.
